...
class TaskPolicy(UtgBasedInputPolicy):
    ...
    def __init__(self, device, app, random_input, task, extracted_info, addiAC: AndroidController, use_memory=False, debug_mode=False):
        ...
        file_handler = logging.FileHandler(f'log_dir_task/{addiAC.emulator_controller.avd_name}_{date_string}_{addiAC.current_episode}.log')
        ...
        try:
            ...
            while input_manager.enabled and self.action_count < input_manager.event_count:
                try:
                    ...
                    self.logger.info(f'StepTaskPolicy start::before input_manager.add_event(event, send_event=True)')
                    input_manager.add_event(event, send_event=True) #output event and its views(.png) into disk
                    self.logger.info(f'StepTaskPolicy start::after input_manager.add_event(event, send_event=True)')
                ...
                #     break
                    ...
                    self.logger.error("exception during sending events: %s" % e)
                    import traceback
                    traceback.print_exc()
            ...
            if self.action_count >= input_manager.event_count:
                ...
                if input_manager.output_dir is not None:
                    ...
                    logger_info_file.writelines(time_str)
                    logger_info_file.close()
            self.logger.info("time cost: %s" % (time.time() - start_time))
        except Exception as e:
            self.logger.exception("exception during sending events: %s" % e)
            raise e
...
class StepTaskPolicy(UtgBasedInputPolicy):
    ...
    def __init__(self, device, app, random_input,  extracted_info, addiAC: AndroidController, step=0):
        ...
        self.task = extracted_info[0]['function']
        ...
        self.__random_explore = random_input
        self.__action_history = []
        self.__state_desc_history = []
        self.__max_state_desc_history = 2
        self.addiAC = addiAC
        self.conversation = ""
    ...
    def start(self, input_manager):
        ...
        time.sleep(2)
        self.device.send_event(ScrollEvent(view=None, direction="UP"))
        time.sleep(2)
        ...
        while input_manager.enabled:#input_manager.event_count:
            try:
                ...
                if self.action_count == 0 and self.master is None:
                    ...
                    if ("system enter" in self.subtask.lower()) or ("layout" in self.subtask.lower()):
                        finish = -1                        
                    else:
                ...
                if finish != -1 and event is not None: # 需要执行事件 ; None对应sleep的空事件
                    ...
                    if self.step > 0: #修正event以外的事件类型，0对应的是task-‘start’ 
                        if (self.extracted_info[self.step-1]['event_or_assertion'] != 'Event') and (finish == 1): #???这里finish==1表示什么
                            ...
                            if "not in the state" in self.subtask:
                                condition = "not in the state"
                        if self.subtask.split()[0].lower() == "clear":
                            condition = "Clear" 
                    self.logger.info(f'StepTaskPolicy start::before input_manager.add_event(event, send_event=True)')
                    input_manager.add_event(event, send_event=True) # execute, maybe set_text_enter and set_text should be divided to execute, and observe and memorize effect of every action. So Eliminate dead action. takes 2 minutes here for kill_app
                    self.logger.info(f'StepTaskPolicy start::after input_manager.add_event(event, send_event=True)')
    ...
    def generate_event_based_on_utg(self, input_manager):
        ...
        #     scrollable_views = current_state.get_scrollable_views()#self._get_scrollable_views(current_state)       
        if len(scrollable_views) > 0: 
            whole_state_views, whole_state_actions, whole_state_strs = self._scroll_in_homepage(current_state, scrollable_views, input_manager)
            ...
            if action is not None and not isinstance(action, list): # action is not a list, but a single action
            ...
            if isinstance(action, list):  # the screen has to be scrolled first
                ...
                for eventid in range(len(action) - 1):
                    # self.device.send_event(action[eventid])
                    input_manager.add_event(action[eventid], send_event=True)
                    time.sleep(2.1)  # wait for the scroll to finish
                    current_state = self.device.get_current_state()
                    self.current_state = current_state
        ...
        #     if action is None:
        #         if self.subtask.split()[0].lower() != "identify":
        #             self.extracted_info[self.step-1]['status'] = -1
        #         finish, action, candidate_actions = self._get_action_with_LLM(current_state, self.__action_history)

        if action is not None: # action is not a list, but a single action
            if action.event_type != "oracle":
                state_desc, _, views_without_id, _ = current_state.get_described_actions()
                if len(self.__state_desc_history) == self.__max_state_desc_history:
                    self.__state_desc_history[self.__max_state_desc_history-1] = state_desc
                else:
                    self.__state_desc_history.append(state_desc)        
    ...
    def  _scroll_in_homepage(self, current_state, scrollable_views, input_manager): #处理scroll的时候，应当区分是局部widget的scroll还是整体页面的scroll，本函数目前只能处理有多个局部 或 只有一个整体 的情况
        '''
        ...
        for scrollerid in range(len(scrollable_views)): # scrollable_views has only one scroller in the home page
            ...
            prefix_scroll_event = self._scroll_to_top(scroller, all_views_for_mark, input_manager)
            ...
            for _ in range(len(prefix_scroll_event)):  # then scroll down ，注意这里是滚动len(prefix_scroll_event)，同上面的_scroll_to_top次数对应;备选方案是根据view的数量来终止滚动
                ...
                input_manager.add_event(ScrollEvent(view=None, direction="DOWN"), send_event=True)
                time.sleep(2.1)  # wait for the scroll to finish
                scrolled_state = self.device.get_current_state()
                ...
                for scrolled_view_id in range(len(scrolled_views)):
                    ...
                    if scrolled_view not in all_views_without_id:
                        scrolled_new_views.append(scrolled_view)
                        all_views_without_id.append(scrolled_view)
                        all_actions.append(prefix_scroll_event_2+[ScrollEvent(view=None, direction="DOWN"), scrolled_candidate_actions[scrolled_view_id]])

                if len(scrolled_new_views) == 0:
                    break
                
                prefix_scroll_event_2.append(ScrollEvent(view=None, direction="DOWN"))
                ...
                self.utg.add_transition(ScrollEvent(view=None, direction="DOWN"), top_state, scrolled_state)
                top_state = scrolled_state
            ...
            all_views_for_mark = []
            _ = self._scroll_to_top(scroller, all_views_for_mark, input_manager, top_state)    
    ...
    def _scroll_to_top(self, scroller, all_views_for_mark, input_manager, old_state=None):
        prefix_scroll_event = []
        if old_state is None:
            old_state = self.current_state 
        for _ in range(MAX_SCROLL_NUM):  # first scroll up to the top           
            # self.device.send_event(ScrollEvent(view=scroller, direction="UP"))
            input_manager.add_event(ScrollEvent(view=None, direction="UP"), send_event=True)
            time.sleep(2.1)  # wait for the scroll to finish
            scrolled_state = self.device.get_current_state()
            self.utg.add_transition(ScrollEvent(view=None, direction="UP"), old_state, scrolled_state) #only store the transition in UTG and state, not store event, should place under "prefix_scroll_event.append(ScrollEvent(view=scroller, direction="UP"))"?
    ...
    def if_action(self, now_action):#now_action:Event Object
        ...
        #action = Edit
        action = self.subtask.split()[0].lower()
        ...
        if action == "click" or action == "swipe":
            ...
            condition = self.subtask.split("\"")[-1]
            if "not" in condition:
                condition = False
            else:
                condition = True
        ...
        str_view_task = self.subtask.split("\"")[1].lower()
        self.logger.info(f'StepTaskPolicy if_action subtask: {str_view_task} item: {str_view}')
        ...
        if "with" in self.subtask:
            input = self.subtask.split("with")[1].split("\"")[1]
        # elif action == "clear":
        #     input = "0"
        return flag, input
    ...
    def _get_action_with_match(self, current_state, action_history):
        ...
        #system enter / back
        if "system" in self.subtask:
            if "back" in self.subtask:
        ...
        self.logger.info(f'StepTaskPolicy _get_action_with_match candidate: {len(view_descs)}——{len(candidate_actions)}——{self.subtask}')
        ...
        for idx in range(0, len(candidate_actions)):
            ...
            desc = desc.replace('\n', '')
            desc += "("+str(idx)+")"
            self.logger.info(f'StepTaskPolicy _get_action_with_match candidate: {idx}——{desc}——{self.subtask}')
        ...
        if (action == "identify") and ("not" in condition):
            ...
            selected_action.view['text'] = self.subtask
            return 1,  selected_action, candidate_actions
    ...
    def _get_after_task(self):
        ...
        for i in range(self.step, len(self.extracted_info)-1): #最后一条task用于验证整个function是否完成，不包含len(self.extracted_info)-1
            desc += f" -subtask {i+1}: {self.extracted_info[i]['subtask']}\n" 
        if desc != "":
            return desc
        else:
    ...
    def _judge_repeated_action(self, action_history: list):
        if len(action_history) >= 2 and len(self.__state_desc_history) >= 2:
            if action_history[-1] == action_history[-2] and self.__state_desc_history[-1] == self.__state_desc_history[-2]:
                return f"Don't choose the action responding to '{action_history[-1]}' that is repeated in the action history."
        return ""
    
    def _swipe_find_app(self, activity, views_without_id):
        if activity == "com.google.android.apps.nexuslauncher/.NexusLauncherActivity":
            if ( "<button text='Voice search<br>Search'></button>" in views_without_id):
                self.home_swipe_up = True
            elif ("<button text='Zoom'>Zoom</button>" in views_without_id):
                self.home_swipe_up = False          
            if self.home_swipe_up:
    ...
    def _get_action_with_LLM(self, action_history, current_state=None, views=None, candidate_actions=None, state_strs=None):
        ...
        question = f"Based on completed actions (pay attention to last few actions), current activity and current state reached so far, I would like to confirm whether I have successfully completed the current '{self.subtask}' subtask. If last few completed actions align with current subtask and lead to expected current activity and state (i.e. current state should not be empty, and last few completed actions lead to expected current state and activity), and no further action in current state is needed to continue with the current subtask, then answer 'yes'; else answer 'no'.  Please provide an answer in 'yes' or 'no'  with brief analysis for this answer. Please format the response as a JSON object with the following keys: 'answer_yes_or_no'(str, 'yes'or'no') 'analysis'(str). " 
        identify_prompt = ""
        if event_or_assertion == "Assertion" and "not" not in self.subtask and self.step != len(self.extracted_info):
            identify_prompt = f"If your answer is yes, please find the corresponding view related to the assertion '{self.subtask}'. Please think step by step in additional analysis for finding view and only return the view's ID. Please supplement the JSON response object with the following key: 'verify_view_id'(int). If view can be found in the current state, choose the view id as verify_view_id; if no proper view can be found in the current state, set verify_view_id as -1.\n"
        if event_or_assertion == "Assertion" and "not" in self.subtask and self.step != len(self.extracted_info):
            identify_prompt = f"If your answer is no, please find the view in the state contradictory to '{self.subtask}' . Please think step by step in analysis for finding view and only return the view's ID. Please supplement the JSON response object with the following key: 'verify_view_id'(int). If view can be found in the current state, choose the view id as verify_view_id; if no proper view can be found in the current state, set verify_view_id as -1.\n"
        ...
        if "verify_view_id" in step:
            action_id = step['verify_view_id']
        ...
        if (step["answer_yes_or_no"] == "yes" and (event_or_assertion != "Assertion" or self.step == len(self.extracted_info))):
            ...
            if "not" not in self.subtask: # should exist in the state
                if action_id != -1:
                    finish = 1
                    ...
                    return finish, selected_action, candidate_actions
                elif self.attempt_count == 0: # 需要找到但是没找到且找了n次应该记录Assertion无效，这里对应n-1;目前设为找n=1次
                    finish = 1
                    selected_action = OracleEvent(
                        condition = self.subtask,
                        assert_accept = False
                    )
                    self.extracted_info = self.extracted_info[:self.step-1] + self.extracted_info[self.step:] # remove the current step from extracted_info at step-1
                    self.addiAC.update_instructions(self.extracted_info)
                    self.step -= 1 # step减1，为start函数中+1做准备
                    return finish, selected_action, candidate_actions
                else: # 继续寻找直到找到或满n次
                    pass
            else: # not in the state, 不需要找; 且回答是yes，表示没有
                finish = 1
                selected_action = OracleEvent(
                        condition = self.subtask,
                        assert_accept = True
        ...
        if (match == NO_ACTION_FOUND or  match == WAIT_ACTION_NUM) and self.step <= len(self.extracted_info)-1: # 一种情况是match == WAIT_ACTION_NUM and self.wait_count==self.wait_max
            ...
            constraints = {
                ...
                "already_completed_or_can_be_advanced_with_action_taken": {
                    "required": True,
                    "type": str,
                    "value_constraints": lambda value: value in ('already_completed', 'can_be_advanced_with_action_taken', 'None')  
                },
                "action_id": {
                    "required": True,
                    "type": int,
            ...
            if self.update_steps_after_looking_after_in_functional_guide or self.update_steps_after_looking_after_matching:
                ...
                if step_list is not None and step_list != []:
                    self.extracted_info = tools.update_reference_steps(self.extracted_info, step_list, self.step)
                    self.addiAC.update_instructions(self.extracted_info)
                    if self.subtask != self.extracted_info[self.step-1]['subtask']: # 更新完步骤后当前步骤
                        self.subtask = self.extracted_info[self.step-1]['subtask']
                        self.attempt_count = 0
            else:
            ...
            if step['subtask_id'] > 1 and step_list is not None and step_list != []: #找到了匹配的子步骤
                if step['already_completed_or_can_be_advanced_with_action_taken'] == "already_completed":
                    finish = 0 # 取0，因为有时候是因为下一个future subtask完成了，大模型会得出前一个future subtask也完成，设置为0可以再确认一遍减少这种情况发生；假如是-1，原因：也看到了当前显示界面，所以没必要再确认一次； self.step = idx已经设定了，后面finish=-1会再向前多挪一个 (这样直接跳转到已完成subtask的下一个subtask)                   
                    return finish, None, candidate_actions
